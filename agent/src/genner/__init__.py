...
from agent.src.genner.Claude import ClaudeGenner
from agent.src.genner.OAI import OAIGenner
from agent.src.genner.OR import OpenRouterGenner

from .Base import Genner
from .Deepseek import DeepseekGenner
from .Qwen import QwenGenner
from agent.tests.mock_genner.MockGenner import MockGenner  # Uncommented for production
	...
	if backend == "deepseek":
		...
		if not or_client:
			...
			return OAIGenner(
				...
				config=OAIConfig(name=openai_config.name, model=os.getenv("AGENT_LLM_MODEL", openai_config.model)),
				stream_fn=stream_fn,
			)
		...
		import os
		model_name = os.getenv("AGENT_LLM_MODEL", "google/gemini-2.0-flash-lite-001")
		gemini_config.name = model_name
