...
from agent.src.agent.trading import TradingAgent
from agent.src.datatypes import (
	StrategyData,
	StrategyDataParameters,
	StrategyInsertData,
	WalletStats,
)
from agent.src.helper import nanoid
...
def assisted_flow(
	...
	summarized_state_change = dedent(f"""
        Holdings Before: {str(start_metric_state).replace(chr(10), "")}
        USD Value Before: {start_metric_state["total_value_usd"]}
        Holdings After: {str(end_metric_state).replace(chr(10), "")}
