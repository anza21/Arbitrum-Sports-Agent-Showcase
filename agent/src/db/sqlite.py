...
from agent.src.datatypes import StrategyData, StrategyInsertData
from agent.src.db.interface import DBInterface
from agent.src.agent_types import ChatHistory
import uuid
...
class SQLiteDB(DBInterface):
	...
	def _init_db(self):
		...
		# Create tables
		with open("/app/agent/src/db/00001_init.sql", "r") as f:
			init_script = f.read()
		# Seed data
		with open("/app/agent/src/db/00002_seed.sql", "r") as f:
			seed_script = f.read()
	...
	def get_pending_bets(self) -> List[Dict[str, Any]]:
		"""Fetch all pending bets from the database.

		Returns:
			List[Dict[str, Any]]: List of pending bet records
		"""
		try:
			with sqlite3.connect(self.db_path) as conn:
				cursor = conn.cursor()
				cursor.execute(
					"""SELECT bet_id, agent_id, market_id, bet_amount, odds, status, created_at 
					   FROM sup_bets 
