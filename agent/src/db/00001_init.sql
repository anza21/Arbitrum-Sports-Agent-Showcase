...
create table if not exists sup_bets (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
create index if not exists idx_bets_agent_status on sup_bets (agent_id, status);
create index if not exists idx_bets_created on sup_bets (created_at);
