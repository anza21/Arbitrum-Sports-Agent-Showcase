...
def main():
    print("--- 🏆 POSTMAN FIXED: Using Correct Data Types from Postman Documentation ---")
    load_dotenv()
    ...
    try:
        ...
        # === STEP 3: EXECUTE TRADE WITH CORRECT DATA TYPES ===
        print("\n--- Step 3: Executing TRADE with Correct Data Types ---")
        sports_amm = w3.eth.contract(address=w3.to_checksum_address(sports_amm_address), abi=SPORTS_AMM_ABI)
        
        # Convert data types according to Postman documentation
        trade_data_for_contract = [{
            "gameId": bytes.fromhex(market['gameId'][2:]),  # Convert hex string to bytes32
        ...
        print(f"Trade Data Types:")
        print(f"  gameId: {type(trade_data_for_contract[0]['gameId'])} = {trade_data_for_contract[0]['gameId']}")
        print(f"  sportId: {type(trade_data_for_contract[0]['sportId'])} = {trade_data_for_contract[0]['sportId']}")
        print(f"  odds: {type(trade_data_for_contract[0]['odds'])} = {trade_data_for_contract[0]['odds']}")
        print(f"  merkleProof: {type(trade_data_for_contract[0]['merkleProof'])} = {trade_data_for_contract[0]['merkleProof']}")
