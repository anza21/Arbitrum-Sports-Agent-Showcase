...
# --- CORRECT ABI from Postman Documentation ---
...
# Simplified ABI for testing - let's try with minimal required fields
...
def main():
    print("--- 🏆 CORRECT ABI: Using Simplified ABI from Postman Docs ---")
    load_dotenv()
    ...
    try:
        ...
        if current_allowance < buy_in_amount_wei:
            print("❌ Insufficient USDC allowance. Need to approve first.")
            return
        else:
            print("✅ USDC allowance sufficient")

        # === STEP 3: EXECUTE TRADE WITH SIMPLIFIED ABI ===
        print("\n--- Step 3: Executing TRADE with Simplified ABI ---")
