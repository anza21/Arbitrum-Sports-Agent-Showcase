...
def main():
    print("--- 🏆 GRAND FINALE on ARBITRUM ---")
    load_dotenv()
    rpc_url = os.getenv('ARBITRUM_RPC_URL'); wallet_address = os.getenv('WALLET_ADDRESS'); private_key = os.getenv('PRIVATE_KEY'); api_key = os.getenv('OVERTIME_REST_API_KEY'); usdc_address = os.getenv('USDC_CONTRACT_ADDRESS'); sports_amm_address = os.getenv('SPORTS_AMM_ADDRESS')
    
    w3 = Web3(Web3.HTTPProvider(rpc_url)); account = w3.eth.account.from_key(private_key)
    print(f"✅ Connected to Arbitrum. Wallet: {account.address}")
    
    market_id = "0x3230323530393132383044444439314400000000000000000000000000000000"; position = 1; buy_in_amount_usd = 3.08
    ...
    try:
        ...
        total_quote = w3.to_wei(quote['quoteData']['totalQuote']['normalizedImplied'], 'ether'); buy_in_amount_wei = int(buy_in_amount_usd * (10**6))
        ...
        w3.eth.wait_for_transaction_receipt(approve_hash); print(f"✅ Approval Confirmed: {approve_hash.hex()}")
        time.sleep(15)

        print("\n--- Step 3: Execute TRADE ---")
        sports_amm = w3.eth.contract(address=w3.to_checksum_address(sports_amm_address), abi=SPORTS_AMM_ABI_MINIMAL)
        trade_data_contract = [{"gameId": market['gameId'],"sportId": market['subLeagueId'],"typeId": market['typeId'],"maturity": market['maturity'],"status": market['status'],"line": int(market['line'] * 100),"playerId": market['playerProps']['playerId'],"odds": [w3.to_wei(o['normalizedImplied'], 'ether') for o in market['odds']],"merkleProof": market['proof'],"position": position,"combinedPositions": market['combinedPositions'],"live": False}]
        trade_tx = sports_amm.functions.trade(trade_data_contract, buy_in_amount_wei, total_quote, w3.to_wei(0.02, 'ether'), '0x0000000000000000000000000000000000000000', w3.to_checksum_address(usdc_address), False).build_transaction({'from': account.address, 'nonce': w3.eth.get_transaction_count(account.address)})
