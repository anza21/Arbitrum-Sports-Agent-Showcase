...
from agent.src.sensor.marketing import MarketingSensor
from agent.src.sensor.trading import TradingSensor
from agent.src.sensor.interface import TradingSensorInterface, MarketingSensorInterface
from agent.src.db import DBInterface
from typing import Callable
from agent.src.agent.marketing import MarketingAgent, MarketingPromptGenerator
from agent.src.agent.trading import TradingAgent, TradingPromptGenerator
...
from agent.src.summarizer import get_summarizer
from anthropic import Anthropic
import docker
from functools import partial
from agent.src.flows.trading import assisted_flow as trading_assisted_flow
from agent.src.flows.marketing import unassisted_flow as marketing_unassisted_flow
from loguru import logger
from agent.src.constants import SERVICE_TO_ENV
from agent.src.constants import FE_DATA_MARKETING_DEFAULTS, FE_DATA_TRADING_DEFAULTS
from agent.src.manager import fetch_default_prompt
from dotenv import load_dotenv
from agent.src.twitter import TweepyTwitterClient
...
def extra_model_questions(answer_model):
	...
	if "openrouter" in answer_model and not os.getenv("OPENROUTER_API_KEY"):
		question_or_key = [
			inquirer.Password(
...
def extra_sensor_questions(answers_agent_type):
	if answers_agent_type == "trading":
		...
		sensor_api_keys = ["ARBITRUM_RPC_URL", "ETHERSCAN_API_KEY"]
		...
		if answer_trading_sensor["sensor"] == "Yes, i have these keys":
			...
			sensor = TradingSensor(
				...
				infura_project_id=os.getenv("ARBITRUM_RPC_URL"),
	...
	elif answers_agent_type == "betting":
		# For betting agent, use real trading sensor with proper configuration
		sensor_api_keys = ["ARBITRUM_RPC_URL", "ETHERSCAN_API_KEY"]
		question_betting_sensor = [
			inquirer.List(
				name="sensor",
				message=f"Do you have these API keys {', '.join(sensor_api_keys)} ?",
				choices=[
		...
		answer_betting_sensor = inquirer.prompt(question_betting_sensor)
		if answer_betting_sensor["sensor"] == "Yes, i have these keys":
			sensor_api_keys = [x for x in sensor_api_keys if not os.getenv(x)]
			question_sensor_api_keys = [
				inquirer.Text(
					name=x, message=f"Please enter value for this variable {x}"
			...
			answer_sensor_api_keys = inquirer.prompt(question_sensor_api_keys)
			eth_address = asyncio.run(
				get_ether_address_from_txn_service("default_betting")
			)
			for x in sensor_api_keys:
				os.environ[x] = answer_sensor_api_keys[x]
			sensor = TradingSensor(
...
def start_betting_agent(
	...
	prompt_generator = BettingPromptGenerator(prompts=fe_data["prompts"])
	...
	# Initialize NewsService for the betting agent
	...
	try:
		...
		if sports_data:
			print(f"✅ Successfully fetched sports data: {len(sports_data)} markets available")
			print("Sample data:", sports_data[:2] if len(sports_data) > 2 else sports_data)
			
			# Call the strategy function
			betting_decisions = agent.formulate_betting_strategy(sports_data)
			...
			print("❌ Failed to fetch sports data from Overtime API")
	except Exception as e:
		print(f"❌ Error in betting workflow: {e}")
	
	print("Betting Agent test completed!")
...
def starter_prompt():
	...
	questions = [
		...
		inquirer.List(
			...
			choices=["trading", "marketing", "betting"],
			default=["trading"],
		),
		inquirer.List(
			name="rag",
	...
	elif answers["agent_type"] == "betting":
		# Load betting configuration from betting.json
		import json
		betting_config_path = os.path.join(os.path.dirname(__file__), "..", "starter", "betting.json")
		try:
			with open(betting_config_path, 'r') as f:
				fe_data = json.load(f)
			logger.info("Successfully loaded betting configuration from betting.json")
		except Exception as e:
			logger.warning(f"Failed to load betting.json, using trading defaults: {e}")
			fe_data = FE_DATA_TRADING_DEFAULTS.copy()
	...
	for x in range(3):
		if answers["agent_type"] == "marketing":
			...
			start_betting_agent(
				agent_type=answers["agent_type"],
				session_id=fe_data.get("agent_id", "default_betting"),
				agent_id=fe_data.get("agent_id", "default_betting"),
				fe_data=fe_data,
				genner=genner,
				db=SQLiteDB(
					db_path=os.getenv("SQLITE_PATH", "../db/superior-agents.db")
